...
export default function Services() {
  const [selected, setSelected] = useState<Dienst | null>(null);
  ...
  return (
    <>
              ...
              return (
                <div key={dienst.id} className="gsap-card">
                  ...
                  >
                    ...
                    {/* Meer info knop */}
                    <button
                      onClick={() => setSelected(dienst)}
                      className="mt-4 inline-flex items-center text-sm font-semibold text-[#06b6d4] hover:text-[#0891b2] transition-colors cursor-pointer"
                    >
                      Meer info →
                    </button>
                  </div>
      ...
      <ServiceModal dienst={selected} onClose={() => setSelected(null)} />
    </>
  );
}
